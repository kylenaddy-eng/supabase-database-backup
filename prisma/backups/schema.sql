...
CREATE OR REPLACE FUNCTION "public"."aggregate_cost_invoices"("p_f_text" "text" DEFAULT ''::"text", "p_f_description" "text" DEFAULT ''::"text", "p_f_treatment" "text" DEFAULT ''::"text", "p_f_status" "text" DEFAULT ''::"text", "p_f_doc_type" "text" DEFAULT ''::"text", "p_f_company" "text" DEFAULT ''::"text", "p_f_from" "date" DEFAULT NULL::"date", "p_f_to" "date" DEFAULT NULL::"date", "p_f_po" "text" DEFAULT ''::"text", "p_f_due_from" "date" DEFAULT NULL::"date", "p_f_due_to" "date" DEFAULT NULL::"date", "p_f_paid" "text" DEFAULT ''::"text", "p_f_cis" "text" DEFAULT ''::"text", "p_f_project" "text" DEFAULT ''::"text", "p_f_check" "text" DEFAULT ''::"text", "p_dup_only" boolean DEFAULT false, "p_missing_due_date" boolean DEFAULT false, "p_f_credit_card" "text" DEFAULT ''::"text", "p_amount_conflict_only" boolean DEFAULT false, "p_payment_month" "text" DEFAULT NULL::"text", "p_overdue_only" boolean DEFAULT false, "p_f_confidential" "text" DEFAULT ''::"text", "p_f_vehicle_service" "text" DEFAULT ''::"text", "p_f_vehicle_id" "uuid" DEFAULT NULL::"uuid") RETURNS "jsonb"
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $_$
  WITH filtered AS (
...
CREATE OR REPLACE FUNCTION "public"."assert_rpc_costs"() RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
  ...
  IF auth.role() IS DISTINCT FROM 'authenticated' OR auth.uid() IS NULL THEN
    RAISE EXCEPTION 'Forbidden';
  END IF;
  IF NOT public.has_permission(auth.uid(), 'access.costs', false) THEN
    RAISE EXCEPTION 'Forbidden';
  END IF;
  RETURN true;
END;
...
ALTER FUNCTION "public"."assert_rpc_costs"() OWNER TO "postgres";
...
BEGIN
  ...
  IF p_action = 'insert' THEN
    IF public.audit_is_submission_parent(p_table) THEN
...
CREATE OR REPLACE FUNCTION "public"."audit_bulk_cost_invoices_summary"("p_ids" "uuid"[], "p_paid" boolean, "p_mode" "text" DEFAULT 'marked_paid'::"text") RETURNS "text"
    LANGUAGE "plpgsql" STABLE
    AS $$
DECLARE
  v_count integer;
...
BEGIN
  ...
  IF v_count = 0 THEN
    RETURN 'No cost invoices changed';
  END IF;

  SELECT array_agg(x.label ORDER BY x.label)
  INTO v_labels
  FROM (
    SELECT public.audit_cost_invoice_label(ci.company_name, ci.invoice_number) AS label
    FROM public.cost_invoices ci
    WHERE ci.id = ANY (p_ids)
    ORDER BY ci.company_name, ci.invoice_number, ci.id
    LIMIT 5
  ) x;

  v_preview := array_to_string(v_labels, ', ');

  IF p_mode = 'restored_paid' THEN
  RETURN format('Restored paid date on %s cost invoice%s%s',
    v_count,
  ...
  IF p_paid THEN
    RETURN format('Marked %s cost invoice%s as paid%s',
      v_count,
      CASE WHEN v_count = 1 THEN '' ELSE 's' END,
      CASE WHEN v_preview <> '' THEN ': ' || v_preview ELSE '' END
    );
  ...
  RETURN format('Marked %s cost invoice%s as unpaid%s',
    v_count,
    CASE WHEN v_count = 1 THEN '' ELSE 's' END,
    CASE WHEN v_preview <> '' THEN ': ' || v_preview ELSE '' END
  );
...
ALTER FUNCTION "public"."audit_bulk_cost_invoices_summary"("p_ids" "uuid"[], "p_paid" boolean, "p_mode" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."audit_cost_invoice_label"("p_company_name" "text", "p_invoice_number" "text") RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
    AS $$
  SELECT coalesce(nullif(trim(p_company_name), ''), 'Unknown supplier')
    || CASE
      WHEN nullif(trim(p_invoice_number), '') IS NOT NULL THEN ' (' || trim(p_invoice_number) || ')'
      ELSE ''
    END;
$$;


ALTER FUNCTION "public"."audit_cost_invoice_label"("p_company_name" "text", "p_invoice_number" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."audit_cost_invoice_labels_json"("p_ids" "uuid"[]) RETURNS "jsonb"
    LANGUAGE "sql" STABLE
    AS $$
  SELECT coalesce(
    jsonb_agg(
      jsonb_build_object(
        'id', ci.id,
        'label', public.audit_cost_invoice_label(ci.company_name, ci.invoice_number)
      )
      ORDER BY ci.company_name, ci.invoice_number, ci.id
    ),
    '[]'::jsonb
  ...
  FROM public.cost_invoices ci
  WHERE p_ids IS NOT NULL AND ci.id = ANY (p_ids);
$$;


ALTER FUNCTION "public"."audit_cost_invoice_labels_json"("p_ids" "uuid"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."audit_enrich_with_entity_label"("p_table" "text", "p_summary" "text", "p_metadata" "jsonb", "p_row" "jsonb") RETURNS TABLE("out_summary" "text", "out_metadata" "jsonb")
    LANGUAGE "plpgsql" STABLE
    AS $$
DECLARE
  v_label text;
...
BEGIN
  ...
  IF public.audit_is_submission_parent(p_table) THEN
    RETURN QUERY SELECT v_summary, v_meta;
    RETURN;
  END IF;

  IF coalesce(v_meta ->> 'bulk_action', '') <> '' THEN
    RETURN QUERY SELECT v_summary, v_meta;
    RETURN;
  END IF;

  v_label := public.audit_entity_label(p_table, p_row);
  IF v_label IS NULL OR trim(v_label) = '' THEN
    RETURN QUERY SELECT v_summary, v_meta;
    RETURN;
  END IF;

  IF v_summary NOT ILIKE '%' || v_label || '%' THEN
    v_summary := v_summary || ' — ' || v_label;
  END IF;

  v_meta := v_meta || jsonb_build_object('entity_label', v_label);
  RETURN QUERY SELECT v_summary, v_meta;
END;
$$;


ALTER FUNCTION "public"."audit_enrich_with_entity_label"("p_table" "text", "p_summary" "text", "p_metadata" "jsonb", "p_row" "jsonb") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."audit_entity_label"("p_table" "text", "p_row" "jsonb") RETURNS "text"
    LANGUAGE "plpgsql" STABLE
    AS $$
DECLARE
  v text;
...
BEGIN
  ...
  CASE p_table
    WHEN 'cost_invoices' THEN
      RETURN public.audit_cost_invoice_label(
        coalesce(p_row ->> 'company_name', ''),
        coalesce(p_row ->> 'invoice_number', '')
      );

    WHEN 'invoices' THEN
      v := coalesce(nullif(trim(p_row ->> 'client_name_snapshot'), ''), 'Unknown client');
      IF nullif(trim(p_row ->> 'invoice_number'), '') IS NOT NULL THEN
        RETURN v || ' (#' || trim(p_row ->> 'invoice_number') || ')';
      END IF;
      RETURN v;

    WHEN 'clients' THEN
      RETURN nullif(trim(p_row ->> 'name'), '');

    WHEN 'projects' THEN
      v := nullif(trim(p_row ->> 'code'), '');
      v_desc := nullif(trim(p_row ->> 'description'), '');
      IF v IS NOT NULL AND v_desc IS NOT NULL THEN
        RETURN v || ' — ' || v_desc;
      END IF;
      RETURN coalesce(v, v_desc);

    WHEN 'vehicles' THEN
      RETURN coalesce(
        nullif(trim(p_row ->> 'registration'), ''),
        nullif(trim(p_row ->> 'description'), '')
      );

    WHEN 'plant' THEN
      v := nullif(trim(p_row ->> 'description'), '');
      v_desc := nullif(trim(p_row ->> 'serial_no'), '');
      IF v IS NOT NULL AND v_desc IS NOT NULL THEN
        RETURN v || ' (' || v_desc || ')';
      END IF;
      RETURN coalesce(v, v_desc);

    WHEN 'subcontractors', 'havs_tools' THEN
      RETURN nullif(trim(p_row ->> 'name'), '');

    WHEN 'profiles' THEN
      v := nullif(trim(p_row ->> 'full_name'), '');
      IF v IS NOT NULL AND nullif(trim(p_row ->> 'username'), '') IS NOT NULL THEN
        RETURN v || ' (@' || trim(p_row ->> 'username') || ')';
      END IF;
      IF v IS NOT NULL THEN
        RETURN v;
      END IF;
      IF nullif(trim(p_row ->> 'username'), '') IS NOT NULL THEN
        RETURN '@' || trim(p_row ->> 'username');
      END IF;
      RETURN NULL;

    WHEN 'holidays' THEN
      IF coalesce(p_row ->> 'start_date', '') <> '' AND coalesce(p_row ->> 'end_date', '') <> '' THEN
        RETURN trim(p_row ->> 'start_date') || ' to ' || trim(p_row ->> 'end_date');
      END IF;
      RETURN NULL;
...
ALTER FUNCTION "public"."audit_entity_label"("p_table" "text", "p_row" "jsonb") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."audit_finalize_submission_summary"("p_table" "text", "p_action" "text", "p_base_summary" "text", "p_row" "jsonb", "p_actor_id" "uuid") RETURNS "text"
    LANGUAGE "plpgsql" STABLE
    AS $$
DECLARE
  v_subject_id uuid;
  ...
  v_summary text;
BEGIN
  IF NOT public.audit_is_submission_parent(p_table) THEN
    RETURN p_base_summary;
  END IF;

  v_form_label := lower(public.audit_submission_form_label(p_table));
  v_subject_id := public.audit_submission_subject_id(p_table, p_row);
  v_subject_name := CASE WHEN v_subject_id IS NOT NULL THEN public.audit_profile_display_name(v_subject_id) ELSE NULL END;
  v_detail := public.audit_submission_detail(p_table, p_row);

  IF p_action = 'insert' THEN
    v_verb := 'Submitted';
  ELSIF p_action = 'delete' THEN
    v_verb := 'Deleted';
  ELSIF p_base_summary ILIKE 'Approved %' OR p_base_summary ILIKE 'Rejected %'
     OR p_base_summary ILIKE 'Submitted %' OR p_base_summary ILIKE 'Resubmitted %' THEN
    v_verb := split_part(p_base_summary, ' ', 1);
    v_summary := p_base_summary;
  ELSE
    v_verb := 'Updated';
  END IF;

  IF v_summary IS NULL THEN
    v_summary := v_verb || ' ' || v_form_label;
  END IF;

  IF v_subject_id IS NOT NULL AND p_actor_id IS NOT NULL AND v_subject_id <> p_actor_id THEN
    v_summary := v_summary || ' on behalf of ' || v_subject_name;
  ELSIF v_subject_id IS NOT NULL AND p_actor_id IS NOT NULL AND v_subject_id = p_actor_id
        AND p_table IN ('rams_briefings', 'toolbox_talks', 'daily_briefings') THEN
  ELSIF v_subject_name IS NOT NULL AND p_table = 'employee_starters' THEN
    v_summary := v_summary || ' for ' || v_subject_name;
  END IF;

  IF v_detail IS NOT NULL THEN
    v_summary := v_summary || ' — ' || v_detail;
  END IF;
...
ALTER FUNCTION "public"."audit_finalize_submission_summary"("p_table" "text", "p_action" "text", "p_base_summary" "text", "p_row" "jsonb", "p_actor_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."audit_is_submission_parent"("p_table" "text") RETURNS boolean
    LANGUAGE "sql" IMMUTABLE
    AS $$
  SELECT p_table = ANY (ARRAY[
    'timesheets', 'plant_inspections', 'vehicle_defects', 'rams_briefings',
    'havs_logs', 'toolbox_talks', 'daily_briefings', 'employee_starters', 'holidays'
  ]::text[]);
$$;


ALTER FUNCTION "public"."audit_is_submission_parent"("p_table" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."audit_profile_display_name"("p_user_id" "uuid") RETURNS "text"
    LANGUAGE "sql" STABLE
    AS $$
  SELECT coalesce(
    nullif(trim(p.full_name), ''),
    CASE WHEN nullif(trim(p.username), '') IS NOT NULL THEN '@' || trim(p.username) ELSE NULL END,
...
ALTER FUNCTION "public"."audit_profile_display_name"("p_user_id" "uuid") OWNER TO "postgres";
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    v_row := to_jsonb(NEW);
    IF public.audit_is_submission_parent(TG_TABLE_NAME) THEN
      v_summary := public.audit_finalize_submission_summary(TG_TABLE_NAME, v_action, v_summary, v_row, v_actor);
      v_metadata := public.audit_submission_metadata(TG_TABLE_NAME, v_row, v_actor);
    END IF;
    SELECT e.out_summary, e.out_metadata
    INTO v_summary, v_metadata
    FROM public.audit_enrich_with_entity_label(TG_TABLE_NAME, v_summary, v_metadata, v_row) e;
    ...
    INSERT INTO public.audit_log (
      ...
      v_actor, v_action, TG_TABLE_NAME, v_entity_id, v_parent_type, v_parent_id, v_summary, v_changes, v_metadata
    );
    RETURN NEW;
...
CREATE OR REPLACE FUNCTION "public"."audit_submission_detail"("p_table" "text", "p_row" "jsonb") RETURNS "text"
    LANGUAGE "plpgsql" STABLE
    AS $$
DECLARE
  v_date date;
BEGIN
  CASE p_table
    WHEN 'timesheets' THEN
      v_date := NULLIF(p_row ->> 'week_ending', '')::date;
      IF v_date IS NOT NULL THEN
        RETURN 'week ending ' || to_char(v_date, 'DD Mon YYYY');
      END IF;
    WHEN 'plant_inspections' THEN
      v_date := NULLIF(p_row ->> 'inspection_date', '')::date;
      IF v_date IS NOT NULL THEN
        RETURN 'inspection ' || to_char(v_date, 'DD Mon YYYY');
      END IF;
    WHEN 'vehicle_defects' THEN
      v_date := NULLIF(p_row ->> 'inspection_date', '')::date;
      IF v_date IS NOT NULL THEN
        RETURN 'inspection ' || to_char(v_date, 'DD Mon YYYY');
      END IF;
    WHEN 'havs_logs' THEN
      v_date := NULLIF(p_row ->> 'log_date', '')::date;
      IF v_date IS NOT NULL THEN
        RETURN 'log ' || to_char(v_date, 'DD Mon YYYY');
      END IF;
    WHEN 'rams_briefings' THEN
      v_date := NULLIF(p_row ->> 'briefing_date', '')::date;
      IF v_date IS NOT NULL THEN
        RETURN 'briefing ' || to_char(v_date, 'DD Mon YYYY');
      END IF;
    WHEN 'toolbox_talks' THEN
      IF coalesce(nullif(trim(p_row ->> 'topic'), ''), '') <> '' THEN
        RETURN 'topic: ' || left(trim(p_row ->> 'topic'), 80);
      END IF;
    WHEN 'daily_briefings' THEN
      IF p_row ->> 'time_delivered' IS NOT NULL THEN
        RETURN 'delivered ' || left(p_row ->> 'time_delivered', 40);
      END IF;
    WHEN 'holidays' THEN
      IF p_row ->> 'start_date' IS NOT NULL AND p_row ->> 'end_date' IS NOT NULL THEN
        RETURN to_char((p_row ->> 'start_date')::date, 'DD Mon YYYY')
          || ' – '
          || to_char((p_row ->> 'end_date')::date, 'DD Mon YYYY');
      END IF;
    WHEN 'employee_starters' THEN
      IF coalesce(nullif(trim(p_row ->> 'full_name'), ''), '') <> '' THEN
        RETURN trim(p_row ->> 'full_name');
      END IF;
    ELSE
      NULL;
  END CASE;
...
ALTER FUNCTION "public"."audit_submission_detail"("p_table" "text", "p_row" "jsonb") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."audit_submission_form_label"("p_table" "text") RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
    AS $$
  SELECT CASE p_table
    WHEN 'timesheets' THEN 'Timesheet'
...
ALTER FUNCTION "public"."audit_submission_form_label"("p_table" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."audit_submission_metadata"("p_table" "text", "p_row" "jsonb", "p_actor_id" "uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql" STABLE
    AS $$
DECLARE
  v_subject_id uuid;
...
BEGIN
  ...
  v_subject_name := CASE WHEN v_subject_id IS NOT NULL THEN public.audit_profile_display_name(v_subject_id) ELSE NULL END;
  v_detail := public.audit_submission_detail(p_table, p_row);
  v_form_label := public.audit_submission_form_label(p_table);

  RETURN jsonb_strip_nulls(jsonb_build_object(
    'form_kind', p_table,
    'form_label', v_form_label,
    'subject_user_id', v_subject_id,
    'subject_name', v_subject_name,
    'on_behalf', CASE
      WHEN v_subject_id IS NOT NULL AND p_actor_id IS NOT NULL AND v_subject_id <> p_actor_id THEN true
      ELSE false
    END,
    'detail', v_detail
  ));
...
ALTER FUNCTION "public"."audit_submission_metadata"("p_table" "text", "p_row" "jsonb", "p_actor_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."audit_submission_subject_id"("p_table" "text", "p_row" "jsonb") RETURNS "uuid"
    LANGUAGE "sql" IMMUTABLE
    AS $$
  SELECT CASE
    WHEN p_table IN ('timesheets', 'plant_inspections', 'vehicle_defects', 'havs_logs') THEN
      NULLIF(p_row ->> 'worker_id', '')::uuid
    WHEN p_table IN ('holidays', 'employee_starters') THEN
      NULLIF(p_row ->> 'user_id', '')::uuid
    WHEN p_table IN ('rams_briefings', 'toolbox_talks', 'daily_briefings') THEN
      NULLIF(p_row ->> 'briefer_id', '')::uuid
    ELSE NULL
  END;
$$;


ALTER FUNCTION "public"."audit_submission_subject_id"("p_table" "text", "p_row" "jsonb") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."can_submit_for"("_owner_id" "uuid") RETURNS boolean
    ...
    SET "search_path" TO 'public'
    AS $$
  SELECT _owner_id = auth.uid() OR public.has_role(auth.uid(), 'admin')
$$;
...
CREATE OR REPLACE FUNCTION "public"."company_resembles_domain"("p_company" "text", "p_domain" "text", "p_canonical" "text" DEFAULT NULL::"text") RETURNS boolean
...
BEGIN
  ...
  v_canon := coalesce(nullif(btrim(p_canonical), ''), nullif(btrim(p_company), ''));
  v_company_key := public.company_match_key(p_company);
  v_canon_key := public.company_match_key(v_canon);

  FOR v_token IN
    SELECT unnest(public.domain_identity_tokens(p_domain))
  LOOP
    IF length(v_token) < 3 THEN
      CONTINUE;
    END IF;

    -- Substring overlap in normalised company / canonical keys.
    IF v_company_key IS NOT NULL AND (
      position(v_token in v_company_key) > 0
      OR (length(v_company_key) >= 3 AND position(v_company_key in v_token) > 0)
    ) THEN
      RETURN true;
    END IF;

    IF v_canon_key IS NOT NULL AND v_canon_key IS DISTINCT FROM v_company_key AND (
      position(v_token in v_canon_key) > 0
      OR (length(v_canon_key) >= 3 AND position(v_canon_key in v_token) > 0)
    ) THEN
      RETURN true;
    END IF;

    -- pg_trgm similarity for longer tokens (e.g. travisperkins).
    IF length(v_token) >= 4 THEN
      IF v_company_key IS NOT NULL
         AND extensions.similarity(v_token, v_company_key) >= 0.45 THEN
        RETURN true;
      END IF;
      IF v_canon_key IS NOT NULL
         AND extensions.similarity(v_token, v_canon_key) >= 0.45 THEN
        RETURN true;
      END IF;
    END IF;

    -- Brand-alias bridge (e.g. holcim.co.uk + H. Sivyer via brand_key holcim).
    IF v_canon IS NOT NULL AND EXISTS (
      SELECT 1
        FROM public.cost_company_brand_aliases ba
       WHERE ba.brand_key = v_token
         AND public.company_match_key(ba.canonical_name) = public.company_match_key(v_canon)
    ) THEN
      RETURN true;
    END IF;
  END LOOP;
...
ALTER FUNCTION "public"."company_resembles_domain"("p_company" "text", "p_domain" "text", "p_canonical" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."cost_invoice_business_dedupe_key"("p_company_invoice_key" "text", "p_invoice_date" "date", "p_total_amount" numeric, "p_po_reference" "text") RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'public'
    AS $$
  SELECT CASE
    WHEN p_company_invoice_key IS NULL
      OR btrim(p_company_invoice_key) = ''
      OR p_invoice_date IS NULL
      OR p_total_amount IS NULL
      OR p_po_reference IS NULL
      OR btrim(p_po_reference) = ''
      OR upper(regexp_replace(btrim(p_po_reference), '\s+', '', 'g')) IN (
        'NA', 'N/A', 'NO-NUMBER', 'NONUMBER', 'NO_NUMBER'
      )
    THEN NULL
    ELSE concat_ws(
      '|',
      p_company_invoice_key,
      p_invoice_date::text,
      public.cost_invoice_norm_amount(p_total_amount),
      upper(regexp_replace(btrim(p_po_reference), '\s+', '', 'g'))
    )
  END;
$$;


ALTER FUNCTION "public"."cost_invoice_business_dedupe_key"("p_company_invoice_key" "text", "p_invoice_date" "date", "p_total_amount" numeric, "p_po_reference" "text") OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."cost_invoices" (
    ...
    "company_invoice_key" "text" GENERATED ALWAYS AS (NULLIF("public"."company_match_key"("company_name"), ''::"text")) STORED,
...
CREATE OR REPLACE FUNCTION "public"."cost_invoice_ci_dedupe_key"("p_company_invoice_key" "text", "p_invoice_number_key" "text") RETURNS "text"
  ...
  SELECT CASE
    ...
    THEN NULL
    ELSE p_company_invoice_key || '|' || p_invoice_number_key
  END;
$$;
  ...
  SELECT nullif(
    regexp_replace(
      concat_ws(
        ...
        public.cost_invoice_norm_amount(p_total_amount),
        public.cost_invoice_norm_text(p_vat_treatment::text),
...
CREATE OR REPLACE FUNCTION "public"."cost_invoice_matches_company_filter"("p_ci" "public"."cost_invoices", "p_f_company" "text") RETURNS boolean
  ...
  SELECT
    ...
    OR public.supplier_match_keys_align(
      public.company_match_key(p_ci.company_name),
      public.company_match_key(p_f_company)
    )
    OR EXISTS (
      SELECT 1
      ...
      WHERE cs.id = p_ci.supplier_id
        AND (
          ...
          OR public.supplier_match_keys_align(
            public.company_match_key(cs.canonical_name),
            public.company_match_key(p_f_company)
          )
        )
    );
...
CREATE OR REPLACE FUNCTION "public"."cost_invoice_matches_project"("p_invoice" "public"."cost_invoices", "p_split_project_id" "uuid", "p_split_project_other" "text", "p_split_is_overhead" boolean, "p_f_project" "text") RETURNS boolean
  ...
  SELECT CASE
    ...
    WHEN p_f_project = '__overhead' THEN
      coalesce(p_invoice.is_overhead, false)
      OR coalesce(p_split_is_overhead, false)
...
CREATE OR REPLACE FUNCTION "public"."cost_invoice_passes_filters"("p_ci" "public"."cost_invoices", "p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text" DEFAULT ''::"text", "p_amount_conflict_only" boolean DEFAULT false, "p_f_payment_reminded" boolean DEFAULT false, "p_payment_reminder_month" "text" DEFAULT NULL::"text", "p_overdue_only" boolean DEFAULT false, "p_f_confidential" "text" DEFAULT ''::"text", "p_f_vehicle_service" "text" DEFAULT ''::"text", "p_f_vehicle_id" "uuid" DEFAULT NULL::"uuid") RETURNS boolean
  ...
  SELECT
    ...
    AND (
      coalesce(nullif(trim(p_f_confidential), ''), NULL) IS NULL
    ...
    AND (
      ...
      OR (
        ...
        AND p_ci.paid_at IS NULL
        AND (
          coalesce(nullif(trim(p_payment_reminder_month), ''), NULL) IS NULL
          OR p_ci.payment_reminder_month = trim(p_payment_reminder_month)
        )
      )
    );
$$;
...
CREATE OR REPLACE FUNCTION "public"."cost_invoice_visible_to_caller"("p_ci" "public"."cost_invoices") RETURNS boolean
    ...
    SET "search_path" TO 'public'
    AS $$
  SELECT NOT p_ci.is_confidential
    OR public.has_permission(auth.uid(), 'costs.view_confidential', false);
$$;


ALTER FUNCTION "public"."cost_invoice_visible_to_caller"("p_ci" "public"."cost_invoices") OWNER TO "postgres";
...
BEGIN
  ...
  IF NEW.company_name IS NOT NULL
    ...
    SELECT r.supplier_id, r.canonical_name
      INTO v_supplier_id, v_canonical
      FROM public.resolve_cost_supplier_for_invoice(
        NEW.supplier_vat_number,
        NEW.supplier_company_reg_number,
        NEW.company_name,
...
CREATE OR REPLACE FUNCTION "public"."domain_identity_tokens"("p_domain" "text") RETURNS "text"[]
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'public'
    AS $$
  SELECT coalesce(array_agg(DISTINCT token ORDER BY token), ARRAY[]::text[])
    FROM (
      SELECT lower(btrim(part)) AS token
        FROM unnest(string_to_array(lower(btrim(coalesce(p_domain, ''))), '.')) AS part
    ) parts
   WHERE token <> ''
     AND token NOT IN (
       'co', 'com', 'org', 'net', 'gov', 'ltd', 'uk', 'io', 'email', 'mail'
     )
     AND token NOT IN (
       'accounts', 'billing', 'sales', 'invoices', 'invoice', 'noreply', 'no-reply',
       'info', 'admin', 'www', 'smtp', 'support', 'enquiries'
     )
$$;


ALTER FUNCTION "public"."domain_identity_tokens"("p_domain" "text") OWNER TO "postgres";
...
BEGIN
  ...
  IF v_key IS NOT NULL AND v_key <> '' THEN
    ...
    SELECT cs.id INTO v_id
      FROM public.cost_suppliers cs
     WHERE public.company_match_key(cs.canonical_name) = v_key
  ...
  INSERT INTO public.cost_suppliers (canonical_name)
  VALUES (v_name)
  ...
  IF v_id IS NOT NULL AND v_key IS NOT NULL AND v_key <> '' THEN
    SELECT cs.canonical_name INTO v_locked_name
      FROM public.cost_suppliers cs
     WHERE cs.id = v_id;

    INSERT INTO public.cost_company_aliases (match_key, canonical_name)
    VALUES (v_key, v_locked_name)
...
BEGIN
  ...
  RETURN net.http_post(
    url := 'https://portal.cjbce.co.uk/api/public/admin/reconcile-all-statements',
    headers := jsonb_build_object('Content-Type', 'application/json', 'apikey', v_apikey),
    body := '{}'::jsonb,
    timeout_milliseconds := 300000
  );
...
ALTER FUNCTION "public"."fire_statement_payment_reconcile_backfill"() OWNER TO "postgres";
...
BEGIN
  ...
  -- Admin-owned holidays remain auto-approved after an edit.
  ...
  -- Admin / matching-subcontractor manager amending someone else's → auto-approved.
  IF auth.uid() IS NOT NULL AND auth.uid() <> NEW.user_id AND (
    public.has_role(auth.uid(), 'admin')
    OR (
      public.has_role(auth.uid(), 'manager')
      AND NOT public.is_staff(NEW.user_id)
      AND public.user_subcontractor(auth.uid()) IS NOT NULL
      AND public.user_subcontractor(auth.uid()) = public.user_subcontractor(NEW.user_id)
    )
  ) THEN
    NEW.status := 'approved';
    NEW.approved_at := now();
  ...
  -- Owner or other edits go back for approval.
  NEW.status := 'pending';
...
CREATE OR REPLACE FUNCTION "public"."holidays_autoapprove"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  -- Admin-owned holidays auto-approve.
  IF public.has_role(NEW.user_id, 'admin') THEN
    NEW.status := 'approved';
  ...
  ELSIF auth.uid() IS NOT NULL AND auth.uid() <> NEW.user_id AND (
    public.has_role(auth.uid(), 'admin')
    OR (
      public.has_role(auth.uid(), 'manager')
      AND NOT public.is_staff(NEW.user_id)
...
BEGIN
  ...
  IF v_norm = ''
     OR v_norm IN ('NA', 'N/A', 'NONUMBER', 'NO_NUMBER', 'NONUMBER') THEN
    RETURN NULL;
  END IF;
  ...
  IF v_alpha IS NOT NULL AND length(v_alpha) >= 2 THEN
    RETURN 'ALPHA:' || v_alpha;
  END IF;
...
BEGIN
  ...
  IF v_domain = 'cjbce.co.uk' OR v_domain LIKE '%.cjbce.co.uk' THEN
    RETURN true;
  END IF;

  SELECT value INTO v_config
    FROM public.app_settings
   WHERE key = 'cost_relay_email_domains';

  IF v_config IS NULL OR btrim(v_config) = '' THEN
    RETURN false;
  END IF;

  FOR v_entry IN
    SELECT lower(btrim(jsonb_array_elements_text(v_config::jsonb)))
  LOOP
    IF v_entry = '' THEN
      CONTINUE;
    END IF;
    IF v_domain = v_entry OR v_domain LIKE '%.' || v_entry THEN
      RETURN true;
    END IF;
  END LOOP;
...
CREATE OR REPLACE FUNCTION "public"."list_audit_log"("p_from" timestamp with time zone DEFAULT NULL::timestamp with time zone, "p_to" timestamp with time zone DEFAULT NULL::timestamp with time zone, "p_actor_id" "uuid" DEFAULT NULL::"uuid", "p_entity_type" "text" DEFAULT ''::"text", "p_action" "text" DEFAULT ''::"text", "p_search" "text" DEFAULT ''::"text", "p_limit" integer DEFAULT 50, "p_offset" integer DEFAULT 0) RETURNS TABLE("id" "uuid", "created_at" timestamp with time zone, "actor_id" "uuid", "actor_username" "text", "actor_full_name" "text", "action" "text", "entity_type" "text", "entity_id" "uuid", "parent_type" "text", "parent_id" "uuid", "summary" "text", "changes" "jsonb", "metadata" "jsonb", "form_label" "text", "subject_full_name" "text", "subject_username" "text", "on_behalf" boolean, "total_count" bigint)
  ...
  WITH filtered AS (
    SELECT
      ...
      coalesce(
        a.metadata ->> 'form_label',
        public.audit_submission_form_label(a.entity_type),
        CASE a.entity_type
          WHEN 'cost_invoices' THEN 'Cost invoice'
          WHEN 'invoices' THEN 'Outgoing invoice'
          WHEN 'profiles' THEN 'User'
          WHEN 'user_roles' THEN 'User role'
          WHEN 'employee_pay' THEN 'Shift rate'
          WHEN 'clients' THEN 'Client'
          WHEN 'projects' THEN 'Project'
          WHEN 'vehicles' THEN 'Vehicle'
      ...
      sp.full_name AS subject_full_name,
      sp.username AS subject_username,
      coalesce((a.metadata ->> 'on_behalf')::boolean, false) AS on_behalf
    FROM public.audit_log a
    LEFT JOIN public.profiles p ON p.id = a.actor_id
    LEFT JOIN public.profiles sp ON sp.id = NULLIF(a.metadata ->> 'subject_user_id', '')::uuid
    WHERE public.has_permission(auth.uid(), 'admin.audit_log', false)
      ...
      AND (
        ...
        OR sp.username ILIKE '%' || trim(p_search) || '%'
        OR sp.full_name ILIKE '%' || trim(p_search) || '%'
        OR coalesce(a.metadata ->> 'subject_name', '') ILIKE '%' || trim(p_search) || '%'
        OR coalesce(a.metadata ->> 'detail', '') ILIKE '%' || trim(p_search) || '%'
        OR coalesce(a.metadata ->> 'form_label', '') ILIKE '%' || trim(p_search) || '%'
        OR coalesce(a.metadata ->> 'entity_label', '') ILIKE '%' || trim(p_search) || '%'
        OR coalesce(a.metadata -> 'invoices'::text, '[]'::jsonb)::text ILIKE '%' || trim(p_search) || '%'
      )
  ),
  counted AS (
...
CREATE OR REPLACE FUNCTION "public"."list_cost_invoice_filter_companies"() RETURNS TABLE("company_name" "text")
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
  WITH auth_guard AS (SELECT public.assert_rpc_costs() AS _)
  SELECT DISTINCT btrim(ci.company_name) AS company_name
    FROM public.cost_invoices ci, auth_guard
   WHERE public.cost_invoice_visible_to_caller(ci)
     AND ci.company_name IS NOT NULL
     AND btrim(ci.company_name) <> ''
     AND upper(btrim(ci.company_name)) <> 'NA'
  UNION
  SELECT DISTINCT btrim(cs.canonical_name) AS company_name
    FROM public.cost_suppliers cs, auth_guard
   WHERE cs.id IN (
           SELECT DISTINCT ci.supplier_id
             FROM public.cost_invoices ci
            WHERE ci.supplier_id IS NOT NULL
              AND public.cost_invoice_visible_to_caller(ci)
         )
     AND cs.canonical_name IS NOT NULL
     AND btrim(cs.canonical_name) <> ''
   ORDER BY 1;
$$;


ALTER FUNCTION "public"."list_cost_invoice_filter_companies"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."list_cost_invoice_filter_options"() RETURNS "jsonb"
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
  WITH auth_guard AS (SELECT public.assert_rpc_costs() AS _),
  invoice_assignments AS (
    SELECT ci.project_id, ci.project_other, ci.is_overhead
    FROM public.cost_invoices ci, auth_guard
    WHERE public.cost_invoice_visible_to_caller(ci)
      AND (ci.project_id IS NOT NULL OR ci.project_other IS NOT NULL OR ci.is_overhead)
  ),
  split_assignments AS (
    SELECT s.project_id, s.project_other, s.is_overhead, s.description
    FROM public.cost_invoice_splits s
    INNER JOIN public.cost_invoices ci ON ci.id = s.cost_invoice_id
    CROSS JOIN auth_guard
    WHERE public.cost_invoice_visible_to_caller(ci)
  ),
  descriptions AS (
    SELECT DISTINCT btrim(s.description) AS description
    FROM split_assignments s
    WHERE s.description IS NOT NULL AND btrim(s.description) <> ''
  ),
  project_ids AS (
    SELECT DISTINCT project_id
    FROM (
      SELECT project_id FROM invoice_assignments WHERE project_id IS NOT NULL
      UNION
      SELECT project_id FROM split_assignments WHERE project_id IS NOT NULL
    ) p
  ),
  other_projects AS (
    SELECT DISTINCT btrim(project_other) AS project_other
    FROM (
      SELECT project_other FROM invoice_assignments
      WHERE project_id IS NULL AND NOT is_overhead AND project_other IS NOT NULL AND btrim(project_other) <> ''
      UNION
      SELECT project_other FROM split_assignments
      WHERE project_id IS NULL AND NOT is_overhead AND project_other IS NOT NULL AND btrim(project_other) <> ''
    ) o
  ),
  overhead_flag AS (
    SELECT
      EXISTS (SELECT 1 FROM invoice_assignments WHERE is_overhead)
      OR EXISTS (SELECT 1 FROM split_assignments WHERE is_overhead) AS has_overhead
  )
  SELECT jsonb_build_object(
    'projectIds', coalesce((SELECT jsonb_agg(project_id ORDER BY project_id) FROM project_ids), '[]'::jsonb),
    'otherProjects', coalesce((SELECT jsonb_agg(project_other ORDER BY project_other) FROM other_projects), '[]'::jsonb),
    'descriptions', coalesce((SELECT jsonb_agg(description ORDER BY description) FROM descriptions), '[]'::jsonb),
    'hasOverhead', (SELECT has_overhead FROM overhead_flag)
  );
$$;
...
CREATE OR REPLACE FUNCTION "public"."list_cost_invoices"("p_f_text" "text" DEFAULT ''::"text", "p_f_description" "text" DEFAULT ''::"text", "p_f_treatment" "text" DEFAULT ''::"text", "p_f_status" "text" DEFAULT ''::"text", "p_f_doc_type" "text" DEFAULT ''::"text", "p_f_company" "text" DEFAULT ''::"text", "p_f_from" "date" DEFAULT NULL::"date", "p_f_to" "date" DEFAULT NULL::"date", "p_f_po" "text" DEFAULT ''::"text", "p_f_due_from" "date" DEFAULT NULL::"date", "p_f_due_to" "date" DEFAULT NULL::"date", "p_f_paid" "text" DEFAULT ''::"text", "p_f_cis" "text" DEFAULT ''::"text", "p_f_project" "text" DEFAULT ''::"text", "p_f_check" "text" DEFAULT ''::"text", "p_dup_only" boolean DEFAULT false, "p_missing_due_date" boolean DEFAULT false, "p_f_credit_card" "text" DEFAULT ''::"text", "p_amount_conflict_only" boolean DEFAULT false, "p_f_payment_reminded" boolean DEFAULT false, "p_payment_reminder_month" "text" DEFAULT NULL::"text", "p_overdue_only" boolean DEFAULT false, "p_f_confidential" "text" DEFAULT ''::"text", "p_f_vehicle_service" "text" DEFAULT ''::"text", "p_f_vehicle_id" "uuid" DEFAULT NULL::"uuid", "p_sort_key" "text" DEFAULT 'received'::"text", "p_sort_dir" "text" DEFAULT 'desc'::"text", "p_limit" integer DEFAULT 20, "p_offset" integer DEFAULT 0, "p_unpaginated" boolean DEFAULT false, "p_cursor_sort_value" "text" DEFAULT NULL::"text", "p_cursor_id" "uuid" DEFAULT NULL::"uuid", "p_skip_total" boolean DEFAULT false) RETURNS TABLE("id" "uuid", "status" "text", "company_name" "text", "company_name_raw" "text", "invoice_number" "text", "po_reference" "text", "invoice_date" "date", "due_date" "date", "due_date_rule" "text", "description" "text", "currency" "text", "net_amount" numeric, "vat_amount" numeric, "total_amount" numeric, "vat_treatment" "text", "nas_path" "text", "nas_fallback_path" "text", "attachment_filename" "text", "attachment_sha256" "text", "source_email_from" "text", "source_subject" "text", "source_message_id" "text", "source_received_at" timestamp with time zone, "gemini_confidence" numeric, "is_duplicate" boolean, "duplicate_of" "uuid", "has_duplicate_siblings" boolean, "has_amount_conflict" boolean, "pending_amount_conflict" "jsonb", "linked_documents" "jsonb", "notes" "text", "created_at" timestamp with time zone, "updated_at" timestamp with time zone, "paid_at" timestamp with time zone, "paid_by_credit_card" boolean, "is_confidential" boolean, "is_vehicle_service" boolean, "vehicle_id" "uuid", "vehicle_registration" "text", "vehicle_registration_extracted" "text", "source_account_id" "uuid", "cis_amount" numeric, "document_type" "text", "project_id" "uuid", "project_other" "text", "is_overhead" boolean, "subcontractor_id" "uuid", "timesheet_check_status" "text", "timesheet_check_at" timestamp with time zone, "timesheet_check_detail" "text", "company_invoice_key" "text", "invoice_number_key" "text", "invoice_format_key" "text", "supplier_email_domain" "text", "supplier_id" "uuid", "supplier_vat_number" "text", "supplier_company_reg_number" "text", "payment_reminded_at" timestamp with time zone, "full_field_dedupe_key" "text", "total_count" bigint)
  ...
  total AS (
    SELECT
      CASE
        WHEN p_skip_total THEN NULL::bigint
  ...
  SELECT
    ...
    o.is_overhead,
    o.subcontractor_id,
    o.timesheet_check_status,
    o.timesheet_check_at,
    o.timesheet_check_detail,
    o.company_invoice_key,
    o.invoice_number_key,
    o.invoice_format_key,
    o.supplier_email_domain,
    o.supplier_id,
    o.supplier_vat_number,
    o.supplier_company_reg_number,
    o.payment_reminded_at,
...
CREATE OR REPLACE FUNCTION "public"."list_cost_supplier_statements"("p_company" "text" DEFAULT ''::"text", "p_received_from" "date" DEFAULT NULL::"date", "p_received_to" "date" DEFAULT NULL::"date", "p_status" "text" DEFAULT ''::"text", "p_limit" integer DEFAULT 20, "p_offset" integer DEFAULT 0, "p_archived_only" boolean DEFAULT false) RETURNS TABLE("id" "uuid", "company_name" "text", "supplier_id" "uuid", "statement_date" "date", "period_from" "date", "period_to" "date", "source_email_from" "text", "source_subject" "text", "source_message_id" "text", "source_received_at" timestamp with time zone, "attachment_sha256" "text", "nas_path" "text", "total_debit" numeric, "total_credit" numeric, "balance" numeric, "lines" "jsonb", "verification_status" "text", "verification_result" "jsonb", "verified_at" timestamp with time zone, "created_at" timestamp with time zone, "archived_at" timestamp with time zone, "superseded_by" "uuid", "total_count" bigint)
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
  WITH filtered AS (
    ...
    WHERE
      ...
      AND (
        (coalesce(p_archived_only, false) AND s.archived_at IS NOT NULL)
        OR (NOT coalesce(p_archived_only, false) AND s.archived_at IS NULL)
      )
  ),
  counted AS (
    SELECT count(*)::bigint AS cnt FROM filtered
...
CREATE OR REPLACE FUNCTION "public"."mark_cost_invoices_paid_by_ids"("p_ids" "uuid"[], "p_paid" boolean DEFAULT true) RETURNS TABLE("updated_count" bigint)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
BEGIN
  ...
  SELECT array_agg(id), count(*)::bigint INTO v_ids, v_count FROM upd;

  IF v_count > 0 AND v_actor IS NOT NULL THEN
    INSERT INTO public.audit_log (actor_id, action, entity_type, summary, changes, metadata)
    VALUES (
      v_actor,
      'update',
      'cost_invoices',
      public.audit_bulk_cost_invoices_summary(v_ids, p_paid, 'marked_paid'),
      jsonb_build_object('invoice_ids', to_jsonb(v_ids)),
      jsonb_build_object(
        'bulk_action', CASE WHEN p_paid THEN 'mark_paid' ELSE 'mark_unpaid' END,
        'count', v_count,
        'invoices', public.audit_cost_invoice_labels_json(v_ids)
      )
    );
  END IF;
...
BEGIN
  ...
  SELECT
    (SELECT array_agg(u.id) FROM upd u),
    (SELECT count(*)::bigint FROM upd u),
    (SELECT entries FROM snap)
  INTO v_ids, v_count, v_snapshot;

  IF coalesce(v_count, 0) > 0 AND v_actor IS NOT NULL THEN
    INSERT INTO public.audit_log (actor_id, action, entity_type, summary, changes, metadata)
    VALUES (
      v_actor,
      'update',
...
BEGIN
  SELECT d.canonical_name
   ...
   WHERE d.domain = p_domain
     AND public.company_resembles_domain(d.canonical_name, p_domain, d.canonical_name)
   ORDER BY
...
CREATE OR REPLACE FUNCTION "public"."refresh_cost_invoice_duplicate_flags"("p_full_keys" "text"[] DEFAULT NULL::"text"[], "p_ci_keys" "text"[] DEFAULT NULL::"text"[], "p_business_keys" "text"[] DEFAULT NULL::"text"[]) RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
  ...
  v_full_refresh boolean;
BEGIN
  v_full_refresh := p_full_keys IS NULL AND p_ci_keys IS NULL AND p_business_keys IS NULL;
...
ALTER FUNCTION "public"."refresh_cost_invoice_duplicate_flags"("p_full_keys" "text"[], "p_ci_keys" "text"[], "p_business_keys" "text"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."refresh_cost_invoice_monthly_rollups"() RETURNS "void"
    LANGUAGE "sql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
  REFRESH MATERIALIZED VIEW CONCURRENTLY public.cost_invoice_monthly_rollups;
$$;
...
BEGIN
  IF p_pdf_name IS NOT NULL
    ...
    IF v_pdf_key IS NOT NULL AND v_pdf_key <> '' THEN
      ...
      SELECT ci.company_name_raw
        INTO v_resolved
        FROM public.cost_invoices ci
       WHERE public.company_match_key(ci.company_name_raw) = v_pdf_key
         AND ci.company_name_raw IS NOT NULL
         AND btrim(ci.company_name_raw) <> ''
         AND upper(btrim(ci.company_name_raw)) <> 'NA'
       GROUP BY ci.company_name_raw
       ORDER BY count(*) DESC, max(ci.created_at) DESC
       LIMIT 1;
      IF v_resolved IS NOT NULL THEN
        RETURN btrim(v_resolved);
      END IF;
    END IF;
  END IF;

  IF p_domain IS NOT NULL AND btrim(p_domain) <> ''
     AND p_format_key IS NOT NULL AND btrim(p_format_key) <> '' THEN
    SELECT ci.company_name
      INTO v_resolved
      FROM public.cost_invoices ci
     WHERE ci.supplier_email_domain = p_domain
       AND ci.invoice_format_key = p_format_key
       AND ci.company_name IS NOT NULL
       AND btrim(ci.company_name) <> ''
       AND upper(btrim(ci.company_name)) <> 'NA'
     GROUP BY ci.company_name
     ...
     LIMIT 1;
    IF v_resolved IS NOT NULL THEN
      IF p_pdf_name IS NULL
         OR btrim(p_pdf_name) = ''
         OR upper(btrim(p_pdf_name)) = 'NA'
  ...
  IF p_format_key IS NOT NULL AND btrim(p_format_key) <> '' THEN
    SELECT count(*)::bigint, count(DISTINCT ci.company_name)::int
      INTO v_total, v_distinct
      FROM public.cost_invoices ci
     WHERE ci.invoice_format_key = p_format_key
       AND ci.supplier_email_domain IS NOT NULL
       AND ci.company_name IS NOT NULL
       AND btrim(ci.company_name) <> ''
       AND upper(btrim(ci.company_name)) <> 'NA';

    IF v_total >= 2 THEN
      SELECT ci.company_name, count(*)::bigint
        INTO v_winner_name, v_winner_cnt
        FROM public.cost_invoices ci
       WHERE ci.invoice_format_key = p_format_key
         AND ci.supplier_email_domain IS NOT NULL
         AND ci.company_name IS NOT NULL
      ...
      IF v_winner_name IS NOT NULL
         AND v_winner_cnt::numeric / v_total::numeric >= 0.8
...
BEGIN
  ...
  IF v_domain IS NULL OR v_domain = ''
     ...
     OR public.is_shared_invoice_portal_domain(v_domain)
     OR public.is_staff_relay_domain(v_domain) THEN
    RETURN NULL;
  END IF;
...
BEGIN
  ...
  SELECT canonical_name INTO v_canon
...
BEGIN
  ...
  v_has_pdf_name := p_name IS NOT NULL
    AND btrim(p_name) <> ''
    AND upper(btrim(p_name)) <> 'NA';
  ...
  IF v_domain IS NOT NULL THEN
    SELECT count(DISTINCT ci.company_name)
      INTO v_domain_cnt
      FROM public.cost_invoices ci
     WHERE ci.supplier_email_domain = v_domain
       AND ci.company_name IS NOT NULL
       AND btrim(ci.company_name) <> ''
       AND upper(btrim(ci.company_name)) <> 'NA';

    IF v_domain_cnt = 1 THEN
      SELECT min(ci.company_name)
        INTO v_domain_only
        FROM public.cost_invoices ci
       WHERE ci.supplier_email_domain = v_domain
         AND ci.company_name IS NOT NULL
...
DECLARE
  ...
  v_fallback text;
BEGIN
  SELECT l.supplier_id, l.canonical_name
    INTO v_supplier_id, v_canonical
    FROM public.lookup_cost_supplier_by_identifiers(p_vat, p_company_reg, p_name) l
   LIMIT 1;
  ...
  IF v_fallback IS NULL
     OR btrim(v_fallback) = ''
     OR upper(btrim(v_fallback)) = 'NA' THEN
    RETURN;
  END IF;
...
CREATE OR REPLACE FUNCTION "public"."revenue_entries_before_insert"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."revenue_entries_before_insert"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."run_statement_payment_reconcile_backfill_once"() RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'net'
    AS $$
DECLARE
  r record;
BEGIN
  PERFORM public.fire_statement_payment_reconcile_backfill();

  FOR r IN SELECT jobid FROM cron.job
    WHERE jobname = 'statement-payment-reconcile-backfill'
  LOOP
...
CREATE OR REPLACE FUNCTION "public"."supplier_match_keys_align"("a" "text", "b" "text") RETURNS boolean
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'public'
    AS $$
  SELECT
...
ALTER FUNCTION "public"."supplier_match_keys_align"("a" "text", "b" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."sync_todo_list_assignments_for_profile"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  -- User joined or moved into a subcontractor group: add to synced lists.
  IF TG_OP = 'INSERT' THEN
  ...
  IF TG_OP = 'UPDATE' THEN
    -- Deactivated: remove from synced lists for current/previous group.
...
CREATE OR REPLACE FUNCTION "public"."trusted_supplier_email_domain"("p_from" "text") RETURNS "text"
    ...
    AS $$
  SELECT CASE
    WHEN public.email_domain(p_from) IS NULL
      OR btrim(public.email_domain(p_from)) = ''
    ...
    THEN NULL
    ELSE public.email_domain(p_from)
  END
$$;
...
BEGIN
  ...
  IF coalesce(v_count, 0) > 0 AND v_actor IS NOT NULL THEN
    ...
    VALUES (
      ...
      'update',
      'cost_invoices',
      public.audit_bulk_cost_invoices_summary(v_ids, true, 'restored_paid'),
      jsonb_build_object('invoice_ids', to_jsonb(v_ids)),
      jsonb_build_object(
        'bulk_action', 'undo_mark_paid',
        'count', v_count,
        'invoices', public.audit_cost_invoice_labels_json(v_ids)
      )
    );
...
CREATE MATERIALIZED VIEW "public"."cost_invoice_monthly_rollups" AS
 SELECT ("date_trunc"('month'::"text", ("invoice_date")::timestamp with time zone))::"date" AS "month_start",
    COALESCE("company_name", 'NA'::"text") AS "company_name",
    "count"(*) AS "invoice_count",
    COALESCE("sum"("net_amount"), (0)::numeric) AS "net_total",
    COALESCE("sum"("vat_amount"), (0)::numeric) AS "vat_total",
    COALESCE("sum"("cis_amount"), (0)::numeric) AS "cis_total",
    COALESCE("sum"(COALESCE("total_amount", ("net_amount" + COALESCE("vat_amount", (0)::numeric)))), (0)::numeric) AS "spend_total",
    COALESCE("sum"(
        CASE
            WHEN ("vat_treatment" = 'standard_20'::"public"."cost_vat_treatment") THEN COALESCE("vat_amount", (0)::numeric)
            ELSE (0)::numeric
        END), (0)::numeric) AS "input_vat_total"
   FROM "public"."cost_invoices" "ci"
  WHERE (("invoice_date" IS NOT NULL) AND ("status" <> 'rejected'::"public"."cost_invoice_status"))
  GROUP BY (("date_trunc"('month'::"text", ("invoice_date")::timestamp with time zone))::"date"), COALESCE("company_name", 'NA'::"text")
  WITH NO DATA;


ALTER MATERIALIZED VIEW "public"."cost_invoice_monthly_rollups" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."cost_supplier_statements" (
    ...
    "superseded_by" "uuid",
    CONSTRAINT "cost_supplier_statements_verification_status_check" CHECK (("verification_status" = ANY (ARRAY['pending'::"text", 'matched_paid'::"text", 'matched_some_paid'::"text", 'matched_not_paid'::"text", 'discrepancies'::"text", 'error'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."daily_briefing_attendees" (
    ...
    "briefing_datetime" timestamp with time zone DEFAULT "now"() NOT NULL,
...
CREATE TABLE IF NOT EXISTS "public"."integration_email_accounts" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "confidential_invoices" boolean DEFAULT false NOT NULL,
...
CREATE TABLE IF NOT EXISTS "public"."integration_portal_accounts" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "label" "text" NOT NULL,
    "portal_type" "text" DEFAULT 'allstar'::"text" NOT NULL,
    ...
    "last_fetch_invoice_count" integer,
...
CREATE TABLE IF NOT EXISTS "public"."invoice_clients" (
    ...
    CONSTRAINT "invoice_clients_billing_mode_check" CHECK (("billing_mode" = ANY (ARRAY['invoice'::"text", 'self_bill'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."notification_preferences" (
    ...
    "user_management" boolean DEFAULT true NOT NULL,
    "team_member_changes" boolean DEFAULT true NOT NULL,
...
CREATE TABLE IF NOT EXISTS "public"."revenue_entries" (
    ...
    "recognition_date" "date" DEFAULT CURRENT_DATE NOT NULL,
    "client_reference" "text",
...
CREATE INDEX "cost_invoices_amount_conflict_idx" ON "public"."cost_invoices" USING "btree" ("has_amount_conflict") WHERE ("has_amount_conflict" = true);



CREATE INDEX "cost_invoices_business_dedupe_key_idx" ON "public"."cost_invoices" USING "btree" ("business_dedupe_key") WHERE ("business_dedupe_key" IS NOT NULL);
...
CREATE INDEX "cost_invoices_confidential_idx" ON "public"."cost_invoices" USING "btree" ("is_confidential") WHERE "is_confidential";
...
CREATE INDEX "cost_invoices_due_date_idx" ON "public"."cost_invoices" USING "btree" ("due_date") WHERE ("due_date" IS NOT NULL);
...
CREATE INDEX "cost_invoices_source_account_id_idx" ON "public"."cost_invoices" USING "btree" ("source_account_id") WHERE ("source_account_id" IS NOT NULL);
...
CREATE INDEX "cost_supplier_statements_active_company_idx" ON "public"."cost_supplier_statements" USING "btree" ("company_name", "source_received_at" DESC NULLS LAST) WHERE ("archived_at" IS NULL);



CREATE INDEX "cost_supplier_statements_archived_at_idx" ON "public"."cost_supplier_statements" USING "btree" ("archived_at") WHERE ("archived_at" IS NOT NULL);
...
CREATE UNIQUE INDEX "cost_suppliers_match_key_unique" ON "public"."cost_suppliers" USING "btree" ("public"."company_match_key"("canonical_name")) WHERE (("public"."company_match_key"("canonical_name") IS NOT NULL) AND ("public"."company_match_key"("canonical_name") <> ''::"text"));
...
CREATE INDEX "revenue_entries_client_id_idx" ON "public"."revenue_entries" USING "btree" ("client_id");



CREATE INDEX "revenue_entries_project_id_idx" ON "public"."revenue_entries" USING "btree" ("project_id");



CREATE INDEX "revenue_entries_recognition_date_idx" ON "public"."revenue_entries" USING "btree" ("recognition_date" DESC);
...
CREATE OR REPLACE TRIGGER "trg_revenue_entries_before_insert" BEFORE INSERT ON "public"."revenue_entries" FOR EACH ROW EXECUTE FUNCTION "public"."revenue_entries_before_insert"();
...
ALTER TABLE ONLY "public"."cost_invoices"
    ADD CONSTRAINT "cost_invoices_source_account_id_fkey" FOREIGN KEY ("source_account_id") REFERENCES "public"."integration_email_accounts"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."cost_supplier_statements"
    ADD CONSTRAINT "cost_supplier_statements_superseded_by_fkey" FOREIGN KEY ("superseded_by") REFERENCES "public"."cost_supplier_statements"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."revenue_entries"
    ADD CONSTRAINT "revenue_entries_client_id_fkey" FOREIGN KEY ("client_id") REFERENCES "public"."invoice_clients"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."revenue_entries"
    ADD CONSTRAINT "revenue_entries_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id") ON DELETE SET NULL;
...
CREATE POLICY "Admins delete cost invoices" ON "public"."cost_invoices" FOR DELETE TO "authenticated" USING (("public"."has_permission"("auth"."uid"(), 'access.costs'::"text", false) AND "public"."cost_invoice_visible_to_caller"("cost_invoices".*)));
...
CREATE POLICY "Admins read cost invoices" ON "public"."cost_invoices" FOR SELECT TO "authenticated" USING (("public"."has_permission"("auth"."uid"(), 'access.costs'::"text", false) AND "public"."cost_invoice_visible_to_caller"("cost_invoices".*)));
...
CREATE POLICY "Admins update cost invoices" ON "public"."cost_invoices" FOR UPDATE TO "authenticated" USING (("public"."has_permission"("auth"."uid"(), 'access.costs'::"text", false) AND "public"."cost_invoice_visible_to_caller"("cost_invoices".*))) WITH CHECK (("public"."has_permission"("auth"."uid"(), 'access.costs'::"text", false) AND "public"."cost_invoice_visible_to_caller"("cost_invoices".*)));
...
CREATE POLICY "cost_invoice_splits admin all" ON "public"."cost_invoice_splits" TO "authenticated" USING (("public"."has_permission"("auth"."uid"(), 'access.costs'::"text", false) AND (EXISTS ( SELECT 1
   FROM "public"."cost_invoices" "ci"
  WHERE (("ci"."id" = "cost_invoice_splits"."cost_invoice_id") AND "public"."cost_invoice_visible_to_caller"("ci".*)))))) WITH CHECK (("public"."has_permission"("auth"."uid"(), 'access.costs'::"text", false) AND (EXISTS ( SELECT 1
   FROM "public"."cost_invoices" "ci"
  WHERE (("ci"."id" = "cost_invoice_splits"."cost_invoice_id") AND "public"."cost_invoice_visible_to_caller"("ci".*))))));
...
CREATE POLICY "revenue_entries invoices permission" ON "public"."revenue_entries" TO "authenticated" USING ("public"."has_permission"("auth"."uid"(), 'access.invoices'::"text", false)) WITH CHECK ("public"."has_permission"("auth"."uid"(), 'access.invoices'::"text", false));
...
CREATE POLICY "starter insert (self or admin on behalf)" ON "public"."employee_starters" FOR INSERT TO "authenticated" WITH CHECK ("public"."can_submit_for"("user_id"));
...
REVOKE ALL ON FUNCTION "public"."aggregate_cost_invoice_summary"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."aggregate_cost_invoice_summary"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."aggregate_cost_invoice_summary"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."aggregate_cost_invoice_summary"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid") TO "service_role";



REVOKE ALL ON FUNCTION "public"."aggregate_cost_invoices"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_payment_month" "text", "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."aggregate_cost_invoices"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_payment_month" "text", "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."aggregate_cost_invoices"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_payment_month" "text", "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."aggregate_cost_invoices"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_payment_month" "text", "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."assert_rpc_costs"() FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."assert_rpc_costs"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."audit_bulk_cost_invoices_summary"("p_ids" "uuid"[], "p_paid" boolean, "p_mode" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."audit_bulk_cost_invoices_summary"("p_ids" "uuid"[], "p_paid" boolean, "p_mode" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."audit_bulk_cost_invoices_summary"("p_ids" "uuid"[], "p_paid" boolean, "p_mode" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."audit_cost_invoice_label"("p_company_name" "text", "p_invoice_number" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."audit_cost_invoice_label"("p_company_name" "text", "p_invoice_number" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."audit_cost_invoice_label"("p_company_name" "text", "p_invoice_number" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."audit_cost_invoice_labels_json"("p_ids" "uuid"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."audit_cost_invoice_labels_json"("p_ids" "uuid"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."audit_cost_invoice_labels_json"("p_ids" "uuid"[]) TO "service_role";



GRANT ALL ON FUNCTION "public"."audit_enrich_with_entity_label"("p_table" "text", "p_summary" "text", "p_metadata" "jsonb", "p_row" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."audit_enrich_with_entity_label"("p_table" "text", "p_summary" "text", "p_metadata" "jsonb", "p_row" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."audit_enrich_with_entity_label"("p_table" "text", "p_summary" "text", "p_metadata" "jsonb", "p_row" "jsonb") TO "service_role";



GRANT ALL ON FUNCTION "public"."audit_entity_label"("p_table" "text", "p_row" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."audit_entity_label"("p_table" "text", "p_row" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."audit_entity_label"("p_table" "text", "p_row" "jsonb") TO "service_role";



GRANT ALL ON FUNCTION "public"."audit_finalize_submission_summary"("p_table" "text", "p_action" "text", "p_base_summary" "text", "p_row" "jsonb", "p_actor_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."audit_finalize_submission_summary"("p_table" "text", "p_action" "text", "p_base_summary" "text", "p_row" "jsonb", "p_actor_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."audit_finalize_submission_summary"("p_table" "text", "p_action" "text", "p_base_summary" "text", "p_row" "jsonb", "p_actor_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."audit_is_submission_parent"("p_table" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."audit_is_submission_parent"("p_table" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."audit_is_submission_parent"("p_table" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."audit_profile_display_name"("p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."audit_profile_display_name"("p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."audit_profile_display_name"("p_user_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."audit_submission_detail"("p_table" "text", "p_row" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."audit_submission_detail"("p_table" "text", "p_row" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."audit_submission_detail"("p_table" "text", "p_row" "jsonb") TO "service_role";



GRANT ALL ON FUNCTION "public"."audit_submission_form_label"("p_table" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."audit_submission_form_label"("p_table" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."audit_submission_form_label"("p_table" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."audit_submission_metadata"("p_table" "text", "p_row" "jsonb", "p_actor_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."audit_submission_metadata"("p_table" "text", "p_row" "jsonb", "p_actor_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."audit_submission_metadata"("p_table" "text", "p_row" "jsonb", "p_actor_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."audit_submission_subject_id"("p_table" "text", "p_row" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."audit_submission_subject_id"("p_table" "text", "p_row" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."audit_submission_subject_id"("p_table" "text", "p_row" "jsonb") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."company_resembles_domain"("p_company" "text", "p_domain" "text", "p_canonical" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."company_resembles_domain"("p_company" "text", "p_domain" "text", "p_canonical" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."company_resembles_domain"("p_company" "text", "p_domain" "text", "p_canonical" "text") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."cost_invoice_business_dedupe_key"("p_company_invoice_key" "text", "p_invoice_date" "date", "p_total_amount" numeric, "p_po_reference" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."cost_invoice_business_dedupe_key"("p_company_invoice_key" "text", "p_invoice_date" "date", "p_total_amount" numeric, "p_po_reference" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."cost_invoice_business_dedupe_key"("p_company_invoice_key" "text", "p_invoice_date" "date", "p_total_amount" numeric, "p_po_reference" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."cost_invoice_business_dedupe_key"("p_company_invoice_key" "text", "p_invoice_date" "date", "p_total_amount" numeric, "p_po_reference" "text") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."cost_invoice_visible_to_caller"("p_ci" "public"."cost_invoices") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."cost_invoice_visible_to_caller"("p_ci" "public"."cost_invoices") TO "authenticated";
GRANT ALL ON FUNCTION "public"."cost_invoice_visible_to_caller"("p_ci" "public"."cost_invoices") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."domain_identity_tokens"("p_domain" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."domain_identity_tokens"("p_domain" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."domain_identity_tokens"("p_domain" "text") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."fire_statement_payment_reconcile_backfill"() FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."fire_statement_payment_reconcile_backfill"() TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."list_cost_invoice_filter_companies"() FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."list_cost_invoice_filter_companies"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."list_cost_invoice_filter_companies"() TO "service_role";



REVOKE ALL ON FUNCTION "public"."list_cost_invoice_filter_options"() FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."list_cost_invoice_filter_options"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."list_cost_invoice_filter_options"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."list_cost_invoices"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_f_payment_reminded" boolean, "p_payment_reminder_month" "text", "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid", "p_sort_key" "text", "p_sort_dir" "text", "p_limit" integer, "p_offset" integer, "p_unpaginated" boolean, "p_cursor_sort_value" "text", "p_cursor_id" "uuid", "p_skip_total" boolean) TO "anon";
GRANT ALL ON FUNCTION "public"."list_cost_invoices"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_f_payment_reminded" boolean, "p_payment_reminder_month" "text", "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid", "p_sort_key" "text", "p_sort_dir" "text", "p_limit" integer, "p_offset" integer, "p_unpaginated" boolean, "p_cursor_sort_value" "text", "p_cursor_id" "uuid", "p_skip_total" boolean) TO "authenticated";
GRANT ALL ON FUNCTION "public"."list_cost_invoices"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_f_payment_reminded" boolean, "p_payment_reminder_month" "text", "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid", "p_sort_key" "text", "p_sort_dir" "text", "p_limit" integer, "p_offset" integer, "p_unpaginated" boolean, "p_cursor_sort_value" "text", "p_cursor_id" "uuid", "p_skip_total" boolean) TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."list_cost_supplier_statements"("p_company" "text", "p_received_from" "date", "p_received_to" "date", "p_status" "text", "p_limit" integer, "p_offset" integer, "p_archived_only" boolean) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."list_cost_supplier_statements"("p_company" "text", "p_received_from" "date", "p_received_to" "date", "p_status" "text", "p_limit" integer, "p_offset" integer, "p_archived_only" boolean) TO "anon";
GRANT ALL ON FUNCTION "public"."list_cost_supplier_statements"("p_company" "text", "p_received_from" "date", "p_received_to" "date", "p_status" "text", "p_limit" integer, "p_offset" integer, "p_archived_only" boolean) TO "authenticated";
GRANT ALL ON FUNCTION "public"."list_cost_supplier_statements"("p_company" "text", "p_received_from" "date", "p_received_to" "date", "p_status" "text", "p_limit" integer, "p_offset" integer, "p_archived_only" boolean) TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."mark_cost_invoices_paid_by_ids"("p_ids" "uuid"[], "p_paid" boolean) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."mark_cost_invoices_paid_by_ids"("p_ids" "uuid"[], "p_paid" boolean) TO "anon";
GRANT ALL ON FUNCTION "public"."mark_cost_invoices_paid_by_ids"("p_ids" "uuid"[], "p_paid" boolean) TO "authenticated";
GRANT ALL ON FUNCTION "public"."mark_cost_invoices_paid_by_ids"("p_ids" "uuid"[], "p_paid" boolean) TO "service_role";



GRANT ALL ON FUNCTION "public"."mark_cost_invoices_paid_filtered"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_paid" boolean, "p_selected_ids" "uuid"[], "p_f_payment_reminded" boolean, "p_payment_reminder_month" "text", "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."mark_cost_invoices_paid_filtered"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_paid" boolean, "p_selected_ids" "uuid"[], "p_f_payment_reminded" boolean, "p_payment_reminder_month" "text", "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."mark_cost_invoices_paid_filtered"("p_f_text" "text", "p_f_description" "text", "p_f_treatment" "text", "p_f_status" "text", "p_f_doc_type" "text", "p_f_company" "text", "p_f_from" "date", "p_f_to" "date", "p_f_po" "text", "p_f_due_from" "date", "p_f_due_to" "date", "p_f_paid" "text", "p_f_cis" "text", "p_f_project" "text", "p_f_check" "text", "p_dup_only" boolean, "p_missing_due_date" boolean, "p_f_credit_card" "text", "p_amount_conflict_only" boolean, "p_paid" boolean, "p_selected_ids" "uuid"[], "p_f_payment_reminded" boolean, "p_payment_reminder_month" "text", "p_overdue_only" boolean, "p_f_confidential" "text", "p_f_vehicle_service" "text", "p_f_vehicle_id" "uuid") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."refresh_cost_invoice_duplicate_flags"("p_full_keys" "text"[], "p_ci_keys" "text"[], "p_business_keys" "text"[]) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."refresh_cost_invoice_duplicate_flags"("p_full_keys" "text"[], "p_ci_keys" "text"[], "p_business_keys" "text"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."refresh_cost_invoice_duplicate_flags"("p_full_keys" "text"[], "p_ci_keys" "text"[], "p_business_keys" "text"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."refresh_cost_invoice_duplicate_flags"("p_full_keys" "text"[], "p_ci_keys" "text"[], "p_business_keys" "text"[]) TO "service_role";



REVOKE ALL ON FUNCTION "public"."refresh_cost_invoice_monthly_rollups"() FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."refresh_cost_invoice_monthly_rollups"() TO "anon";
GRANT ALL ON FUNCTION "public"."refresh_cost_invoice_monthly_rollups"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."refresh_cost_invoice_monthly_rollups"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."revenue_entries_before_insert"() TO "anon";
GRANT ALL ON FUNCTION "public"."revenue_entries_before_insert"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."revenue_entries_before_insert"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."supplier_match_keys_align"("a" "text", "b" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."supplier_match_keys_align"("a" "text", "b" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."supplier_match_keys_align"("a" "text", "b" "text") TO "service_role";
...
GRANT ALL ON TABLE "public"."cost_invoice_monthly_rollups" TO "anon";
GRANT ALL ON TABLE "public"."cost_invoice_monthly_rollups" TO "authenticated";
GRANT ALL ON TABLE "public"."cost_invoice_monthly_rollups" TO "service_role";
